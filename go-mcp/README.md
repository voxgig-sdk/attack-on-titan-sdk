...
[MCP](https://modelcontextprotocol.io) server exposing the AttackOnTitan SDK as
two agent tools — `attack-on-titan_list` and `attack-on-titan_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/attack-on-titan-mcp)
make build

# 2. Provide credentials via the environment
export ATTACK_ON_TITAN_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user attack-on-titan \
  -- /absolute/path/to/attack-on-titan-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./attack-on-titan-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// attack-on-titan_list: first page of records
{ "entity": "character" }
{ "entity": "character", "query": { } }

// attack-on-titan_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/attack-on-titan-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user attack-on-titan \
     -- "$PWD"/dist/*/attack-on-titan-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `attack-on-titan_list` and `attack-on-titan_load` tools now appear
   in new sessions. Ask the agent to *"list character using attack-on-titan"*
   and it calls `attack-on-titan_list` with `{"entity":"character"}`.
...
```sh
export ATTACK_ON_TITAN_APIKEY=sk_live_xxx            # API key
export ATTACK_ON_TITAN_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `attack-on-titan_list` tool
...
### Call the `attack-on-titan_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `attack-on-titan_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `attack-on-titan_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `ATTACK_ON_TITAN_APIKEY` | API key sent with every request. |
| `ATTACK_ON_TITAN_BASE` | Optional override of the API base URL. |
...
```sh
./attack-on-titan-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"attack-on-titan_load","arguments":{"entity":"character","query":{"id":1}}}}'
```
