...
func TestTitanDirect(t *testing.T) {
	t.Run("direct-list-titan", func(t *testing.T) {
		setup := titanDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-titan", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-titan", func(t *testing.T) {
		setup := titanDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-titan", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type titanDirectSetupResult struct {
	client *sdk.AttackOnTitanSDK
	calls  *[]map[string]any
	live   bool
	idmap  map[string]any
}

func titanDirectSetup(mockres any) *titanDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"ATTACK_ON_TITAN_TEST_TITAN_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["ATTACK_ON_TITAN_TEST_TITAN_ENTID"]; ok {
		...
		return &titanDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &titanDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
