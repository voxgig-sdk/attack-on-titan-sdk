...
import (
	...
	omni "github.com/voxgig-sdk/attack-on-titan-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.AttackOnTitanError); is {
				out := map[string]any{
					"name":    "AttackOnTitanError",
...
func omniCtx(arg any, client *sdk.AttackOnTitanSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.AttackOnTitanSDK); is {
			client = live
