...
func TestTitanEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Titan(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Titan(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		titanRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.titan")))
		...
		titanRef01DataDt0LoadResult := core.ToMapAny(entityData(titanRef01DataDt0Loaded))
...
func titanBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("ATTACK_ON_TITAN_TEST_TITAN_ENTID")
	...
	idmapResolved := core.ToMapAny(env["ATTACK_ON_TITAN_TEST_TITAN_ENTID"])
