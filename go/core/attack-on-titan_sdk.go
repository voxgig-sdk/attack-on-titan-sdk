...
func (sdk *AttackOnTitanSDK) OptionsMap() map[string]any {
...
func (sdk *AttackOnTitanSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *AttackOnTitanSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *AttackOnTitanSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *AttackOnTitanSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *AttackOnTitanSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *AttackOnTitanSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("AttackOnTitanSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *AttackOnTitanSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *AttackOnTitanSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("AttackOnTitanSDK: graphql: %s", msg)
		res["graphql"] = errors
	}
...
func TestSDK(testopts map[string]any, sdkopts map[string]any) *AttackOnTitanSDK {
	if sdkopts == nil {
	...
	sdk := NewAttackOnTitanSDK(sdkopts)
	sdk.Mode = "test"
