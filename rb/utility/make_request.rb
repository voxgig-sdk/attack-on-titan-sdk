...
module AttackOnTitanUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = AttackOnTitanResponse.new({})
    ...
    if fetch_err
      ...
      response = AttackOnTitanResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = AttackOnTitanResponse.new(fetched)
    else
