...
class TitanEntity
  ...
  def make
    opts = @_entopts.dup
    TitanEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = AttackOnTitanHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Titan, Hash] the current Titan data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = AttackOnTitanHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Titan fields)
  def match_get
  ...
  # Load a single Titan.
  #
  # @param reqmatch [TitanLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Titan.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Titan, Hash] the loaded Titan; raises AttackOnTitanError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = AttackOnTitanHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [TitanListMatch, Hash, nil] match filter (any subset of
  #   Titan fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Titan>, Array] the matching Titan items; raises AttackOnTitanError on failure
