...
class AttackOnTitanSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = AttackOnTitanHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = AttackOnTitanHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, AttackOnTitanFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    AttackOnTitanUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    params = AttackOnTitanHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = AttackOnTitanHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = AttackOnTitanHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = AttackOnTitanError.new(
        "graphql_error", "AttackOnTitanSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = AttackOnTitanSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
