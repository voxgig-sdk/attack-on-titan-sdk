// Command AttackOnTitan-cli is a CLI + REPL for the AttackOnTitan SDK,
...
const prompt = "attack-on-titan"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "character episode location organization titan"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      ATTACK_ON_TITAN_APIKEY (api key), ATTACK_ON_TITAN_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: ATTACK_ON_TITAN_APIKEY carries the API key and
	// ATTACK_ON_TITAN_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("ATTACK_ON_TITAN_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("ATTACK_ON_TITAN_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewAttackOnTitanSDK(opts)
