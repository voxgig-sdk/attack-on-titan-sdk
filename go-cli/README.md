...
boru-driven command-line client **and** interactive REPL for the AttackOnTitan
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/attack-on-titan-cli)
make build

# 2. See usage (words, entities, env vars)
./attack-on-titan-cli --help

# 3. Provide credentials once, via the environment
export ATTACK_ON_TITAN_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./attack-on-titan-cli list character
./attack-on-titan-cli load 1 character            # {id:1} shorthand
./attack-on-titan-cli load '{id:1}' character       # explicit match map
./attack-on-titan-cli list episode

# 5. Override the API base URL for a single call
ATTACK_ON_TITAN_BASE=https://api.example.com ./attack-on-titan-cli list character

# 6. No arguments -> interactive REPL
./attack-on-titan-cli
attack-on-titan> list character
attack-on-titan> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/attack-on-titan-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/attack-on-titan-cli list character
   ```
...
```sh
./attack-on-titan-cli load 1 character          # scalar shorthand for {id:1}
./attack-on-titan-cli load '{id:1}' character     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `attack-on-titan>`). Each line is
evaluated as its own boru expression:

```text
$ ./attack-on-titan-cli
attack-on-titan> list character
attack-on-titan> /help
attack-on-titan> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/attack-on-titan-cli`. |
...
(`./attack-on-titan-cli <expr>`) and interactively (the REPL), and expressions compose the
