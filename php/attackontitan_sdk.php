...
class AttackOnTitanSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = AttackOnTitanHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = AttackOnTitanHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!AttackOnTitanFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, AttackOnTitanFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return AttackOnTitanUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = AttackOnTitanHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = AttackOnTitanHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = AttackOnTitanHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new AttackOnTitanError($op . "_allow",
                "AttackOnTitanSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = AttackOnTitanHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new AttackOnTitanError("graphql_error",
                "AttackOnTitanSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    // Canonical facade: $client->Titan()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->titan()
    // resolves here too.
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new AttackOnTitanSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
