...
// AttackOnTitan SDK Titan entity
...
class TitanEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new TitanEntity($this->_client, $opts);
    }

    /**
     * @param Titan|array $args Titan data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = AttackOnTitanHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Titan|array The current Titan data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Titan fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = AttackOnTitanHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Titan fields).
     */
     ...
     * Load a single Titan.
     *
     * @param TitanLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed TitanLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Titan|array The loaded Titan as an assoc-array at the
     *   SDK boundary; throws AttackOnTitanError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = AttackOnTitanHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
    ...
    /**
     * List Titan items matching the given filter.
     *
     * @param TitanListMatch|array|null $reqmatch Match filter (any subset
     *   of Titan fields) as an assoc-array; TitanListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Titan[]|array A list of Titan items as assoc-arrays at
     *   the SDK boundary; throws AttackOnTitanError on failure (item-5 convention).
     */
