...
// AttackOnTitan SDK retry feature
...
class AttackOnTitanRetryFeature extends AttackOnTitanBaseFeature
{
    ...
    public function init(AttackOnTitanContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (AttackOnTitanContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(AttackOnTitanContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(AttackOnTitanContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
