...
// AttackOnTitan SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class AttackOnTitanTestFeature extends AttackOnTitanBaseFeature
{
    ...
    public function init(AttackOnTitanContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (AttackOnTitanContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = AttackOnTitanParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (AttackOnTitanContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(AttackOnTitanContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = AttackOnTitanParam::call($ctx, $k);
