...
class AttackOnTitanMakeError
{
    public static function call(?AttackOnTitanContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new AttackOnTitanContext([], null);
        }
        $op = $ctx->op ?? new AttackOnTitanOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new AttackOnTitanResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof AttackOnTitanError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new AttackOnTitanError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof AttackOnTitanError) {
