...
// AttackOnTitan SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class AttackOnTitanMakePoint
{
    public static function call(AttackOnTitanContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof AttackOnTitanError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $point = $p;
                $select_def = AttackOnTitanHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = AttackOnTitanHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
