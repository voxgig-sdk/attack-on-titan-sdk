...
// AttackOnTitan SDK pipeline test
//
...
{
    private static function utility(): AttackOnTitanUtility
    {
        return new AttackOnTitanUtility();
    }
    ...
    private static function ctx(array $over = []): AttackOnTitanContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new AttackOnTitanContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new AttackOnTitanOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof AttackOnTitanError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new AttackOnTitanOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = AttackOnTitanMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new AttackOnTitanOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = AttackOnTitanMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new AttackOnTitanOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = AttackOnTitanMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new AttackOnTitanSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = AttackOnTitanMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new AttackOnTitanResponse([]),
            'result' => new AttackOnTitanResult([]),
        ]);
        [, $err] = AttackOnTitanMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new AttackOnTitanSpec([]),
            'response' => null,
            'result' => new AttackOnTitanResult([]),
        ]);
        [, $err] = AttackOnTitanMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new AttackOnTitanSpec(['step' => 's']),
            'response' => new AttackOnTitanResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new AttackOnTitanResult([]),
        ]);
        [, $err] = AttackOnTitanMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new AttackOnTitanSpec(['step' => 's']),
            'response' => new AttackOnTitanResponse(self::resp(200, ['v' => 1])),
            'result' => new AttackOnTitanResult([]),
        ]);
        [, $err] = AttackOnTitanMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new AttackOnTitanSpec(['step' => 's']),
            'response' => new AttackOnTitanResponse(self::resp(200, ['v' => 2])),
            'result' => new AttackOnTitanResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        AttackOnTitanMakeResponse::call($ctx);
    ...
    {
        $preset = new AttackOnTitanResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new AttackOnTitanSpec([]),
            'response' => new AttackOnTitanResponse([]),
            'result' => new AttackOnTitanResult([]),
        ...
        [$got, $err] = AttackOnTitanMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new AttackOnTitanResult([])]);
        [, $err] = AttackOnTitanMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new AttackOnTitanSpec([]), 'result' => null]);
        [, $err] = AttackOnTitanMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new AttackOnTitanSpec(['step' => 's']),
            'result' => new AttackOnTitanResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new AttackOnTitanOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = AttackOnTitanMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new AttackOnTitanSpec(['step' => 's']),
            'result' => new AttackOnTitanResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new AttackOnTitanOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = AttackOnTitanMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new AttackOnTitanResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new AttackOnTitanSpec([]),
            'result' => new AttackOnTitanResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = AttackOnTitanMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = AttackOnTitanMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new AttackOnTitanError('boom', 'boom');
        $utility->fetcher = function (AttackOnTitanContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new AttackOnTitanSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = AttackOnTitanMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (AttackOnTitanContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = AttackOnTitanMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(AttackOnTitanResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        AttackOnTitanMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (AttackOnTitanContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new AttackOnTitanSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = AttackOnTitanMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new AttackOnTitanResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new AttackOnTitanSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = AttackOnTitanMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = AttackOnTitanMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = AttackOnTitanMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new AttackOnTitanResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, AttackOnTitanDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new AttackOnTitanResult(['ok' => false])]);
        $this->expectException(AttackOnTitanError::class);
        AttackOnTitanDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new AttackOnTitanResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', AttackOnTitanMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        AttackOnTitanMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new AttackOnTitanBaseFeature();
        $b = new AttackOnTitanBaseFeature();
        AttackOnTitanFeatureAdd::call($ctx, $a);
        AttackOnTitanFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): AttackOnTitanBaseFeature
    {
        $f = new AttackOnTitanBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        AttackOnTitanFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): AttackOnTitanContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new AttackOnTitanSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = AttackOnTitanPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        AttackOnTitanPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new AttackOnTitanResponse(['headers' => null]),
            'result' => new AttackOnTitanResult([]),
        ]);
        AttackOnTitanResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        AttackOnTitanResultBody::call($ctx);
