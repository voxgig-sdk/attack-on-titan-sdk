...
class TitanEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = AttackOnTitanSDK::test($seed, null);
        $seen = iterator_to_array($base->Titan(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Titan(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set ATTACK_ON_TITAN_TEST_TITAN_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $titan_ref01_data_dt0_load_result = Helpers::to_map(is_object($titan_ref01_data_dt0_loaded) && method_exists($titan_ref01_data_dt0_loaded, 'data_get') ? $titan_ref01_data_dt0_loaded->data_get() : $titan_ref01_data_dt0_loaded);
...
function titan_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("ATTACK_ON_TITAN_TEST_TITAN_ENTID");
