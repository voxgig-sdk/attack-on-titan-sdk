...
// AttackOnTitan SDK feature test
//
...
class FtCtrl extends AttackOnTitanControl
{
...
class FtClient
{
    ...
    private AttackOnTitanUtility $utility;

    public function __construct(AttackOnTitanUtility $utility, array $options)
    {
    ...
    public function get_utility(): AttackOnTitanUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (AttackOnTitanContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public AttackOnTitanContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (AttackOnTitanContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = AttackOnTitanConfig::make_feature($name);
    ...
    private static function buildUrl(AttackOnTitanSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof AttackOnTitanError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new AttackOnTitanResponse($fetched);
        }
    ...
    private function fail(AttackOnTitanContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (AttackOnTitanContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
