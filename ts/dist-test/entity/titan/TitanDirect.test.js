...
(0, node_test_1.describe)('TitanDirect', async () => {
    ...
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.AttackOnTitanSDK({
    ...
    (0, node_test_1.test)('direct-load-titan', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-titan', setup.live))
            return;
    ...
    (0, node_test_1.test)('direct-list-titan', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-titan', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function liveScenariosActive() { return false && process.env.ATTACK_ON_TITAN_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.AttackOnTitanSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['ATTACK_ON_TITAN_TEST_TITAN_ENTID'];
    ...
    const client = new __1.AttackOnTitanSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=TitanDirect.test.js.map