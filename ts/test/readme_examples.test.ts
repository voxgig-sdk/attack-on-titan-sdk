...
//     (\`new AttackOnTitanSDK(...)\` / \`AttackOnTitanSDK.test(...)\` become
//     \`AttackOnTitanSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'AttackOnTitanSDK'
...
const TEST_SEED = {"entity":{"character":{"test01":{"id":"test01"}},"episode":{"test01":{"id":"test01"}},"location":{"test01":{"id":"test01"}},"organization":{"test01":{"id":"test01"}},"titan":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new AttackOnTitanSDK(...)\` and any
// \`AttackOnTitanSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+AttackOnTitanSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/AttackOnTitanSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
