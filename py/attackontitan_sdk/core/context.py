...
from attackontitan_sdk.core.helpers import get_ctx_prop, to_map
...
class AttackOnTitanContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = AttackOnTitanControl()
        ...
        if isinstance(sp, AttackOnTitanSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, AttackOnTitanResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, AttackOnTitanResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return AttackOnTitanError(code, msg, self)
