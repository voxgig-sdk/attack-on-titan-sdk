...
from attackontitan_sdk.core.response import AttackOnTitanResponse
from attackontitan_sdk.core.operation import AttackOnTitanOperation
...
def _err_from_map(m):
    ...
    if msg == "":
        return None
    code = m.get("code", "")
    return AttackOnTitanError(code, msg)
...
class TestPrimaryUtility:
    ...
    def test_done_basic(self):
        ...
        def subject(entry):
            ...
            except AttackOnTitanError as e:
                return None, e
    ...
    def test_make_error_no_throw(self):
        ...
        ctx.result = AttackOnTitanResult({
            "ok": False,
            "resdata": {"id": "safe01"},
        })
    ...
    def test_feature_init_basic(self):
        init_client = AttackOnTitanSDK.test(None, None)
        ...
        class TestInitFeature(AttackOnTitanBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "initfeat"
                self.active = True
    ...
    def test_feature_init_inactive(self):
        ...
        class TestInitFeatureInactive(AttackOnTitanBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "nofeat"
                self.active = False
    ...
    def test_fetcher_blocked_test_mode(self):
        ...
        blocked_client = AttackOnTitanSDK({
            "base": "http://localhost:8080",
            "system": {
                "fetch": mock_fetch,
            },
    ...
    def test_make_result_no_result(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = AttackOnTitanSpec({"step": "start"})
        ctx.result = None
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = AttackOnTitanSDK.test(None, setup_opts)
    ...
    def test_operator_basic(self):
        ...
        def subject(entry):
            ...
            if not isinstance(in_val, dict):
                in_val = {}
            op = AttackOnTitanOperation(in_val)
            return {
                "entity": op.entity,
                "name": op.name,
                "input": op.input,
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = AttackOnTitanSDK.test(None, setup_opts)
