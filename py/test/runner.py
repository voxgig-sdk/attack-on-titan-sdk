...
class AttackOnTitanTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            AttackOnTitanTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = AttackOnTitanTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = AttackOnTitanTestRunner.getenv("ATTACK_ON_TITAN_TEST_LIVE")
        override = AttackOnTitanTestRunner.getenv("ATTACK_ON_TITAN_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = AttackOnTitanTestRunner.getenv(key)
        ...
        explain = AttackOnTitanTestRunner.getenv("ATTACK_ON_TITAN_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if AttackOnTitanTestRunner._test_control is not None:
            return AttackOnTitanTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                AttackOnTitanTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            AttackOnTitanTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = AttackOnTitanTestRunner.load_test_control()
...
def load_env_local():
    AttackOnTitanTestRunner.load_env_local()


def env_override(m):
    return AttackOnTitanTestRunner.env_override(m)


def entity_data(v):
    return AttackOnTitanTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return AttackOnTitanTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return AttackOnTitanTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return AttackOnTitanTestRunner.load_test_control()


def live_client_options():
    return AttackOnTitanTestRunner.live_client_options()


def live_delay_ms():
    return AttackOnTitanTestRunner.live_delay_ms()
