...
class TestTitanEntity:
    ...
    def test_should_stream(self):
        ...
        base = AttackOnTitanSDK.test(seed, None)
        seen = list(base.Titan(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = AttackOnTitanSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.Titan(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _titan_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "titan." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set ATTACK_ON_TITAN_TEST_TITAN_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        titan_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.titan")))
        titan_ref01_data = None
        if len(titan_ref01_data_raw) > 0:
        ...
        titan_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(titan_ref01_data_dt0_loaded))
        assert titan_ref01_data_dt0_load_result is not None
        assert titan_ref01_data_dt0_load_result["id"] == titan_ref01_data["id"]



def _titan_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/titan/TitanTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "ATTACK_ON_TITAN_TEST_TITAN_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("ATTACK_ON_TITAN_TEST_TITAN_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("ATTACK_ON_TITAN_TEST_LIVE") == "TRUE":
        merged_opts = vs.merge([
            {
            },
            extra or {},
        ])
        client = AttackOnTitanSDK(helpers.to_map(merged_opts))

    _live = env.get("ATTACK_ON_TITAN_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("ATTACK_ON_TITAN_TEST_EXPLAIN") == "TRUE",
        "live": _live,
