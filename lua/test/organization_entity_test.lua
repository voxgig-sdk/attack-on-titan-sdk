...
describe("OrganizationEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set ATTACKONTITAN_TEST_ORGANIZATION_ENTID JSON to run live")
      return
    end
    ...
    assert.is_nil(err)
    local organization_ref01_data_dt0_load_result = helpers.to_map(organization_ref01_data_dt0_loaded)
...
function organization_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("ATTACKONTITAN_TEST_ORGANIZATION_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["ATTACKONTITAN_TEST_ORGANIZATION_ENTID"])
