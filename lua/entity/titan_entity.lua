...
local TitanEntity = {}
TitanEntity.__index = TitanEntity


function TitanEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TitanEntity)
  self._name = "titan"
  self._client = client
...
function TitanEntity:get_name()
  return self._name
end


function TitanEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TitanEntity.new(self._client, opts)
end
...
function TitanEntity:mark_deleted()
  self._deleted = true
end


function TitanEntity:deleted()
  return true == self._deleted
end


function TitanEntity:data_set(args)
  if args ~= nil then
...
function TitanEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TitanEntity:match_set(args)
  if args ~= nil then
...
function TitanEntity:match_get()
...
function TitanEntity:stream(action, args, callopts)
...
function TitanEntity:_run_op(ctx, post_done)
  local utility = self._utility
