...
local AttackOnTitanSDK = {}
AttackOnTitanSDK.__index = AttackOnTitanSDK
...
AttackOnTitanSDK._make_feature = _make_feature


function AttackOnTitanSDK.new(options)
  local self = setmetatable({}, AttackOnTitanSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function AttackOnTitanSDK:options_map()
...
function AttackOnTitanSDK:get_utility()
  return Utility.copy(self._utility)
end


function AttackOnTitanSDK:get_root_ctx()
  return self._rootctx
end


function AttackOnTitanSDK:prepare(fetchargs)
...
function AttackOnTitanSDK:direct(fetchargs)
...
-- by passing it.
function AttackOnTitanSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function AttackOnTitanSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "AttackOnTitanSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function AttackOnTitanSDK:Titan(data)
  ...
  if data == nil then
    if self._titan == nil then
      self._titan = EntityMod.new(self, nil)
    end
    return self._titan
  end
  return EntityMod.new(self, data)
...
function AttackOnTitanSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = AttackOnTitanSDK.new(sdkopts)
  sdk.mode = "test"
